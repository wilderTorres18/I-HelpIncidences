<template>
  <div>
    ...
    <div class="bg-white rounded-md shadow overflow-hidden max-w-full">
      <form @submit.prevent="update">
        <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
          ...
          <div class="pr-6 pb-8 w-full lg:w-1/3 flex justify-end">
            <div class="btn-indigo p-3 rounded-full cursor-pointer ml-auto" @click="addNew">
              <icon class="w-5 h-5 fill-gray-100" name="plus" />
            </div>
        ...
        <div class="flex flex-col mb-5 text-sm">
          <div v-for="(lan, li) in form.language_values" :key="li" class="p-8 -mr-6 -mb-8 flex flex-wrap md:flex-nowrap items-center lang_input">
            <text-input v-model="lan.name" class="pr-6 pb-2 w-full lg:w-1/2" label="Title" />
            <text-input v-model="lan.value" class="pr-6 pb-2 w-full lg:w-1/2" label="Value" />
      ...
      <div v-if="new_lang_form" class="note-form-overlay" />
      <div v-if="new_lang_form" id="modal" class=" note-form py-8 transition duration-150 ease-in-out z-10 absolute top-0 right-0 bottom-0 left-0">
        <div role="alert" class="container mx-auto">
          <form @submit.prevent="store()">
            <div class="relative py-10 px-6 md:px-10 bg-white shadow-md rounded border border-gray-400">
              ...
              <span class="cursor-pointer absolute top-0 right-0 mt-4 mr-5 text-gray-400 hover:text-gray-600 transition duration-150 ease-in-out rounded focus:ring-2 focus:outline-none focus:ring-gray-600" @click="new_lang_form = false">
