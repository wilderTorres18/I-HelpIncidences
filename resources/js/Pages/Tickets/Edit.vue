                ...
                <form @submit.prevent="update" class="bg-white rounded-md shadow overflow-hidden mr-2">
                    <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
                        ...
                        <select-edit-input placeholder="Search customer" :onInput="doFilter" :items="customers"
                                           ...
                                           :value="ticket.user" :editable="user_access.ticket.update && ticket.status.slug !== 'closed'">
                        </select-edit-input>
                        ...
                        <select-edit-input placeholder="Search priority" :items="priorities"
                                           ...
                                           :value="ticket.priority" :editable="user_access.ticket.update && ticket.status.slug !== 'closed'">
                        </select-edit-input>
                        <select-edit-input v-if="!(hidden_fields && hidden_fields.includes('department'))" placeholder="Search department" :items="departments"
                                           ...
                                           :value="ticket.department" :editable="user_access.ticket.update && ticket.status.slug !== 'closed'">
                        </select-edit-input>

                        <select-edit-input v-if="!(hidden_fields && hidden_fields.includes('assigned_to'))" placeholder="Search user" :onInput="doFilterUsersExceptCustomer" :items="usersExceptCustomers"
                                           v-model="form.assigned_to" :error="form.errors.assigned_to"
                                           class="pr-6 pb-8 w-full lg:w-1/3" :label="__('Assigned to')"
                                           :value="ticket.assigned_user??'Not Assigned'" :editable="user_access.ticket.update && ticket.status.slug !== 'closed'">
                        </select-edit-input>
                        ...
                        <select-edit-input placeholder="Select status to change" :items="statuses"
                                           ...
                                           :value="ticket.status?ticket.status.name:'N/A'" :editable="ticket.status.slug !== 'closed'">
                        </select-edit-input>
                        ...
                        <select-edit-input v-if="!(hidden_fields && hidden_fields.includes('ticket_type'))" placeholder="Search type" :items="types"
                                           ...
                                           :value="ticket.type" :editable="user_access.ticket.update && ticket.status.slug !== 'closed'">
                        </select-edit-input>
                        ...
                        <select-edit-input v-if="!(hidden_fields && hidden_fields.includes('category'))" placeholder="Search category" :items="categories"
                                           ...
                                           :value="ticket.category" :editable="user_access.ticket.update && ticket.status.slug !== 'closed'">
                        </select-edit-input>
                        ...
                        <text-input v-if="user_access.ticket.update" v-model="form.subject" :error="form.errors.subject" class="pr-6 pb-8 w-full lg:w-2/3" :label="__('Subject')" />
                        ...
                        <div class="pr-6 pb-8 w-full lg:w-full flex-col">
                            ...
                            <div v-if="form.files.length" class="flex items-center justify-between pr-6 pt-8 w-full lg:w-1/2" v-for="(file, fi) in form.files" :key="fi">
                                ...
                                <button v-if="ticket.status && ticket.status.slug !== 'closed'" type="button" class="btn flex justify-center items-center" @click="fileRemove(file, fi)">
                                    {{ __('Remove') }}</button>
                            </div>
                        </div>

                        <div class="pr-6 pb-8 w-full lg:w-1/3 flex items-center" v-if="ticket.status && ticket.status.slug === 'closed' && ticket.review">
                        ...
                        <div class="flex flex-col w-full" v-if="ticket.status && ticket.status.slug === 'closed' && !ticket.review && auth.user.role.slug === 'customer'">
