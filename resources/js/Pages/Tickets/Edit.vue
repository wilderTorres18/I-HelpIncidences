            ...
            <div class="max-w-full lg:w-3/5">
                <form @submit.prevent="update" class="bg-white rounded-md shadow overflow-hidden mr-2">
                    <div class="p-8 -mr-6 -mb-8 flex flex-wrap">
                        <!--Super Admin -->
                        <select-edit-input placeholder="Search customer" :onInput="doFilter" :items="customers"
                        ...
                        <select-edit-input v-if="!(hidden_fields && hidden_fields.includes('department'))"
                                           placeholder="Search department" :items="departments"
                        ...
                        <select-edit-input v-if="!(hidden_fields && hidden_fields.includes('assigned_to'))"
                                           placeholder="Search user" :onInput="doFilterUsersExceptCustomer"
                        ...
                        <select-edit-input v-if="!(hidden_fields && hidden_fields.includes('ticket_type'))"
                                           placeholder="Search type" :items="types"
                        ...
                        <select-edit-input v-if="!(hidden_fields && hidden_fields.includes('category'))"
                                           placeholder="Search category" :items="categories"
                        ...
                        <text-input v-if="user_access.ticket.update" v-model="form.subject" :error="form.errors.subject"
                        ...
                        <div class="pr-6 pb-8 w-full lg:w-full flex-col">
                            <button type="button" class="btn flex justify-center items-center pb-3 border-0 pl-0"
                                    @click="fileBrowse">
                                <icon name="file" class="flex-shrink-0 h-5 fill-gray-400 pr-1"/>
                                <strong>{{ __('Attach File') }}</strong>
                            </button>
                            <div v-if="attachments.length" class="flex items-center justify-between pr-6 pt-8 w-full"
                                 v-for="(file, fi) in attachments" :key="fi">
                                <div class="flex-1 pr-1">
                                    {{ file.name }} <span class="text-gray-500 text-xs">({{
                                        getFileSize(file.size)
                                    }})</span> <a v-if="file.user" class="text-sm"
                                                  :href="this.route('users.edit', file.user.id)">{{
                                ...
                                <div class="a__buttons flex justify-end items-center ">
                                    ...
                                    <button type="button" class="btn flex items-center ml-3"
                            ...
                            <div v-if="form.files.length"
                                ...
                                <button v-if="ticket.status && ticket.status.slug !== 'closed'" type="button"
                        ...
                        <div class="pr-6 pb-8 w-full lg:w-1/3 flex items-center"
                        ...
                        <div class="flex flex-col w-full"
                            ...
                            <textarea-input v-model="form.review" :error="form.errors.review"
                    ...
                    <div class="px-8 py-4 bg-gray-50 border-t border-gray-100 flex items-center">
                        <button v-if="user_access.ticket.delete" class="text-red-600 hover:underline" tabindex="-1"
                ...
                <div class="bg-white rounded-md shadow overflow-hidden ml-2 chat-area comment-box flex-1 flex flex-col">
                    <div class="flex-3">
                        <div class="chat-header flex flex-col pb-3">
                            ...
                            <p class="text-sm font-light">
                                {{ __('Comment histories for this ticket will be available here.') }}</p>
                        </div>
                    </div>
                        ...
                        <div v-for="(comment, index) in comments.slice().reverse()" :key="index"
                            ...
                            <div v-if="comment.user_id !== user.id" class="flex-2">
                                <div class="w-12 h-12 relative">
                                    <span class="w-12 h-12 rounded-full mx-auto user_icon" alt="chat-user"
                                          v-if="comment.user">{{ Array.from(comment.user.first_name)[0] }}</span>
                                </div>
                            </div>
                            <div v-if="comment.user_id !== user.id" class="flex-1 px-2">
                                ...
                                <div class="pl-4"><small
                                    class="text-gray-500">{{ moment(comment.updated_at).fromNow(true) }}</small></div>
                            </div>
                    ...
                    <div class="flex-2 pt-4 pb-3">
                        <div class="write bg-white shadow flex rounded-lg">
                            ...
                            <div class="flex-1">
                                <textarea v-model="comment" name="message" @keydown.enter.exact.prevent="submitComment"
                                          class="w-full block outline-none py-4 px-4 text-sm bg-transparent overflow-hidden"
                                          rows="1" :placeholder="__('Escribe un comentario y presiona enter para enviar...')"
                                          autofocus></textarea>
                            </div>
                            <div class="flex-2 w-35 p-2 flex content-center items-center">
                                ...
                                <div class="flex-1">
                                    <button class="bg-blue w-10 h-10 rounded-full flex justify-center items-center"
                                            @click="submitComment">
