<template>
  <div class="text-base text-black dark:text-white dark:bg-slate-900 layout_landing" :class="current_mode" :dir="$page.props.dir">
    ...
    <!-- Back To Top Start -->
    <a id="back-to-top" href="javascript:void(0)" class="back-to-top flex fixed hidden bottom-5 right-5 left-auto z-[999] h-10 w-10 items-center justify-center rounded-md bg-primary text-white shadow-md transition duration-300 ease-in-out hover:bg-dark" @click="topFunction">
        ...
        <div class="flex justify-center items-center ">
          <div class="w-80 h-96 bg-white rounded shadow-2xl">
            <nav class="w-full bg-gray-900 rounded-tr rounded-tl flex justify-between items-center">
              ...
              <div class="flex items-center"> <i class="mdi mdi-video text-gray-300 mr-4" /> <i class="mdi mdi-phone text-gray-300 mr-2" /> <i class="mdi mdi-dots-vertical text-gray-300 mr-2" /> </div>
            </nav>
            <div id="journal-scroll" class="overflow-auto px-1 py-1 reverse__order" style="height: 19rem;">
              <div v-for="(message, index) in current_chat.messages.slice().reverse()" v-if="current_chat.messages && current_chat.messages.length" :key="index" class="flex items-end pr-4 mt-4" :class="{ 'pt-3 pl-10 justify-end': !!message.contact_id }">
                ...
                <span class="flex h-auto text-gray-200 text-md font-normal rounded-sm px-1 p-1 items-end agent_text" :class="{ 'ml-1 bg-gray-900 dark:bg-slate-800': !!message.user_id, 'bg-green-900': !!message.contact_id }">
                  {{ message.message }}
                </span>
            ...
            <div class="flex justify-between items-center p-1 bg-white chat_b_footer">
              <div class="relative t__area">
                ...
                <input
                  id="typemsg" v-model="message" type="text" class=" pl-3 pr-3 py-2 focus:outline-none h-12 border-gray-100"
                  placeholder="Type and press [enter].." @keydown.enter.exact.prevent="sendMessage"
                />
                <i class="mdi mdi-paperclip absolute right-8 top-1 transform -rotate-45 text-gray-400" />
              </div>
                  ...
                  <form class="bg-white rounded px-8 pt-6 pb-8">
                    <div class="flex justify-center mb-4">
                      <div class="flex flex-col pr-2">
                        ...
                        <input id="firstName" v-model="init_user.firstName" class=" appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none" type="text" placeholder="First name" />
                      </div>
                      <div class="flex flex-col pl-2">
                        <label class="block text-gray-700 text-xs font-bold mb-2" for="lastName">
                          Apellidos completos
                        </label>
                        <input id="lastName" v-model="init_user.lastName" class=" appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none" type="text" placeholder="Last name" />
                      </div>
                    </div>
                    <div class="mb-6">
                      ...
                      <input id="emailAddress" v-model="init_user.email" class=" appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none" type="text" placeholder="Email address" />
                    </div>
                    <div class="flex items-center">
